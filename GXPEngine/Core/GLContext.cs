...
namespace GXPEngine.Core
{
	...
	public class GLContext
	{
		...
		public void DrawQuad(Vec2[] verts, float[] transform, float[] uvs, Colour colour)
		{
			...
			GL.glUniformMatrix4fv(GL.glGetUniformLocation(shaderPrograms[0], "transform"),
				1, false, transform);

			GL.glUniform4f(GL.glGetUniformLocation(shaderPrograms[0], "tint"),
				colour.rf, colour.gf, colour.bf, colour.af);
		...
		public void DrawLine(Vec2 start, Vec2 end, Colour colour, float width)
		{
			...
			GL.glUniform4f(GL.glGetUniformLocation(shaderPrograms[1], "color"), //Shaders use the American spelling of color.
				colour.rf, colour.gf, colour.bf, colour.af);
