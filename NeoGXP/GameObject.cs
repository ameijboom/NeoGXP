...
//TODO: Look over these duplicate methods again and perhaps ask Paul what he meant by them.
...
public abstract class GameObject : Transformable
{
	...
	/// </summary>
	public static Game game => Game.main;
	...
	/// </summary>
	public IEnumerable<GameObject> GetCollisions (bool includeTriggers = true, bool includeSolid = true)
	{
	...
	/// When not inside the NeoGXP package, specify the parameter as NeoGXP.Core.GLContext.
	...
	/// </summary>
	public virtual float TimeOfImpact (GameObject other, float vx, float vy, out Vec2 normal) {
		normal = new Vec2 ();
		...
		float TOI=collider.TimeOfImpact (other.collider,
			//p1.x-p0.x, p1.y-p0.y,
	...
	/// </summary>
	public virtual Collision MoveUntilCollision(float vx, float vy, IEnumerable<GameObject> objectsToCheck) {
		Collision col = null;
		float minTOI = 1;
		foreach (GameObject other in objectsToCheck) {
			if (other.collider is {isTrigger: true}) continue;
			float newTOI = TimeOfImpact (other, vx, vy, out Vec2 newNormal);
	...
	public virtual Collision MoveUntilCollision(float vx, float vy) {
		x += vx;
		y += vy;
		IEnumerable<GameObject> overlaps = GetCollisions (false,true);
		x -= vx;
		y -= vy;
	...
	public override Vec2 TransformPoint(float x, float y)
	{
		...
		return parent?.TransformPoint (ret.x, ret.y) ?? ret;
	}
	...
	public override Vec2 TransformDirection(float x, float y)
	{
		...
		return parent?.TransformDirection (ret.x, ret.y) ?? ret;
	}
	...
	public override Vec2 InverseTransformPoint(float x, float y)
	{
		...
		return parent?.InverseTransformPoint (ret.x, ret.y) ?? ret;
	}
	...
	public override Vec2 InverseTransformDirection(float x, float y)
	{
		...
		return parent?.InverseTransformDirection (ret.x, ret.y) ?? ret;
	}
	...
	{
		return GetType() == type ? this
			: _children.Select(child => child.FindObjectOfType(type)).FirstOrDefault(result => result != null);
	}
	...
	public T FindObjectOfType<T>() where T:GameObject {
		...
		return _children.Select(child => child.FindObjectOfType<T>()).FirstOrDefault(result => result != null);
	}
