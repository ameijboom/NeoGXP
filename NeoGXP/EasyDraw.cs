...
{
	private static readonly SKFont DefaultFont = new(SKTypeface.FromFamilyName("Noto Sans"), 15);
	...
	{
		font = new SKFont (SKTypeface.FromFamilyName(fontName, style ?? SKFontStyle.Normal), pointSize);
	}
	...
	public void Text(string text, bool clear=false, byte clearAlpha=0, byte clearRed=0, byte clearGreen=0, byte clearBlue=0) {
		...
		float tx = 0;
		float ty = 0;
		tx = HorizontalTextAlign switch
		{
			CenterMode.Center => Texture.width / 2f,
			CenterMode.Max => Texture.width,
			_ => tx,
		};
		ty = VerticalTextAlign switch
		{
			CenterMode.Center => Texture.height / 2f,
			CenterMode.Max => Texture.height,
			_ => ty,
		};
		Text(text, tx, ty);
	}
