...
public class Sprite : GameObject
{
	...
	protected virtual void SetUVs() {
		...
		UVs = new float[8] { left, top, right, top, right, bottom, left, bottom };
	}

	public float[] GetUVs(bool safe=true) {
		if (safe) {
			return (float[])UVs.Clone();
		} else {
			return UVs;
		}
	}
	...
	public virtual int width {
		get {
			if (Texture != null) return (int)Math.Abs(Texture.width * _scaleX);
			return 0;
		}
		set {
			if (Texture != null && Texture.width != 0) scaleX = value / (float)Texture.width;
		}
	}
	...
	public virtual int height {
		get {
			if (Texture != null) return (int)Math.Abs(Texture.height * _scaleY);
			return 0;
		}
		set {
			if (Texture != null && Texture.height != 0) scaleY = value / (float)Texture.height;
		}
	}
	...
	protected override void RenderSelf(GLContext glContext)
	{
		...
		glContext.SetColor((byte)((_color >> 16) & 0xFF),
			...
			(byte)(_color & 0xFF),
			(byte)(alpha * 0xFF));
		glContext.DrawQuad(GetExtents(), UVs);
	...
	public Vec2[] GetExtents() {
		Vec2[] ret = new Vec2[4];
		ret[0] = TransformPoint(Bounds.left, Bounds.top);
		ret[1] = TransformPoint(Bounds.right, Bounds.top);
		ret[2] = TransformPoint(Bounds.right, Bounds.bottom);
		ret[3] = TransformPoint(Bounds.left, Bounds.bottom);
		return ret;
	}
