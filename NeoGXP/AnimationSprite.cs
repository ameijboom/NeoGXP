...
public class AnimationSprite : Sprite
{
	...
	protected void initializeAnimFrames(int cols, int rows, int frames=-1)
	{
		...
		Bounds = new Rectangle(0, 0, Texture.width * _frameWidth, Texture.height * _frameHeight);
	...
	public override int width {
		get {
			if (Texture != null) return (int)Math.Abs(Texture.width * _scaleX * _frameWidth);
			return 0;
		}
		set {
			if (Texture != null && Texture.width != 0) scaleX = value / (Texture.width * _frameWidth);
		}
	}
	...
	public override int height {
		get {
			if (Texture != null) return (int)Math.Abs(Texture.height * _scaleY * _frameHeight);
			return 0;
		}
		set {
			if (Texture != null && Texture.height != 0) scaleY = value / (Texture.height * _frameHeight);
		}
	}
	...
	protected override void SetUVs() {
		...
		if (!game.PixelArt) {
			//fix1
			float wp = .5f / Texture.width;
			left += wp;
			right -= wp;
			//end fix1

			//fix2
			float hp = .5f / Texture.height;
			top += hp;
			bottom -= hp;
			//end fix2
		}
	...
	public void Animate(float deltaFrameTime=1) {
		...
		if (!(_animationFrameCounter >= _animationDelay)) return;
		NextFrame();
