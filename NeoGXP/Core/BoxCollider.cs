...
public class BoxCollider : Collider
{
	...
	{
		if (other is not BoxCollider collider) return false;
		Vec2[] c = _owner.GetExtents();
		if (c == null) return false;
		Vec2[] d = collider._owner.GetExtents();
	...
	//														areaOverlap()
	//------------------------------------------------------------------------------------------------------------------------
	private static bool AreaOverlap(IList<Vec2> c, IList<Vec2> d) {
		// normal 1:
	...
	/// </summary>
	private static bool PointOverlapsArea(Vec2 p, IList<Vec2> c) {
		...
		float t = ((p.y - c[0].y) * dx1 - (p.x - c[0].x) * dy1) / dot;
		if (t is > 1 or < 0) return false;
	...
	public override float TimeOfImpact (Collider other, float vx, float vy, out Vec2 normal) {
		normal = new Vec2 ();
		if (other is not BoxCollider collider) return float.MaxValue;
	...
	/// <param name="cx">corner point X of body 1</param>
	/// <param name="cy">corner point Y of body 1</param>
	/// <param name="nx">current normal (not necessarily normalized)</param>
	/// <param name="ny">current normal (not necessarily normalized)</param>
	/// <param name="dx">body vector of body 1 that gives the max depth along this normal</param>
	/// <param name="dy">body vector of body 1 that gives the max depth along this normal</param>
	/// <param name="d">points of body 2</param>
	/// <param name="vx">relative velocity of body 2 w. resp. to body 1</param>
	/// <param name="vy">relative velocity of body 2 w. resp. to body 1</param>
	/// <param name="maxTOI">time of impact. Updated when we find better values along this normal.</param>
	/// <param name="minTOE">time of exit. Updated when we find better values along this normal.</param>
	/// <returns>Returns true if the TOI is updated.</returns>
	private static bool UpdateImpactExitTime(float cx, float cy, float nx, float ny, float dx, float dy, Vec2[] d, float vx, float vy, ref float maxTOI, ref float minTOE) {
		float dot = dy * ny + dx * nx;
		...
		{
			if (!(minT >= 1) && !(maxT < 0)) return false; // no overlap in this direction, ever.
	...
	public override Collision GetCollisionInfo (Collider other)
	{
		...
		Vec2 normal=new();
		Vec2 point=new();
		if (other is not BoxCollider collider) return null;
	...
	private static bool UpdateCollisionPoint(
		float cx, float cy, float nx, float ny, float dx, float dy, IList<Vec2> d, bool invertNormal,
