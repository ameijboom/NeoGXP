...
public class Gizmos {
	...
	public static void DrawLine(float x1, float y1, float x2 = 0, float y2 = 0, GameObject space = null, uint color = 0, byte width = 0) {
		...
		if (space == null) {
			_instance._drawCalls.Add(new DrawLineCall(x1, y1, x2, y2, color, width));
		} else {
			// transform to the given parent space:
			Vec2 start = space.TransformPoint(x1, y1);
			Vec2 end = space.TransformPoint(x2, y2);

			_instance._drawCalls.Add(new DrawLineCall(start.x, start.y, end.x, end.y, color, width));
		}
	}
	...
	private void DrawLines(GLContext glContext)
	{
		if (_drawCalls.Count <= 0) return;
		foreach (DrawLineCall dc in _drawCalls) {
