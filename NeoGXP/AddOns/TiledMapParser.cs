...
public static class MapParser {
	private static readonly XmlSerializer Serial = new(typeof(Map));
...
public class Map : PropertyContainer {
	...
	public override string ToString() {
		...
		output += "TILE LAYERS:\n";
		output = Layers.Aggregate(output, (current, l) => current + l);

		output += "IMAGE LAYERS:\n";
		output = ImageLayers.Aggregate(output, (current, l) => current + l);

		output += "TILE SETS:\n";
		output = TileSets.Aggregate(output, (current, t) => current + t);

		output += "OBJECT GROUPS:\n";

		return ObjectGroups.Aggregate(output, (current, g) => current + g);
	}
...
public class PropertyContainer {
	...
	{
		return propertyList != null && propertyList.properties.Any(p => p.Name == key && p.Type == type);
	}
	...
	/// Otherwise, it returns the default value that you can pass as second parameter.
	/// The returned color can be set directly as color value of a NeoGXP Sprite.
	/// </summary>
...
public class Layer : PropertyContainer {
	...
	private void FillTileArray(short[,] sgrid=null, uint[,] uigrid=null) {
...
public class PropertyList {
	...
	public override string ToString()
	{
		return properties.Aggregate("", (current, p) => current + p);
	}
}
...
public class ObjectGroup : PropertyContainer {
	...
	public override string ToString() {
		...
		return Objects.Aggregate(output, (current, obj) => current + obj);
	}
}
...
public class TiledUtils {
	/// <summary>
	/// This translates a Tiled color string to a uint that can be used as a NeoGXP Sprite color.
	/// </summary>
	public static uint GetColor(string htmlColor)
	{
		return htmlColor.Length switch
		{
			9 => (uint) ((Convert.ToInt32(htmlColor.Substring(3, 2), 16) << 24) + // R
			...
			7 => (uint) ((Convert.ToInt32(htmlColor.Substring(1, 2), 16) << 24) + // R
			             (Convert.ToInt32(htmlColor.Substring(3, 2), 16) << 16) + // G
			             (Convert.ToInt32(htmlColor.Substring(5, 2), 16) << 8) + // B
			             0xFF),
			_ => throw new Exception("Cannot recognize color string: " + htmlColor),
		};
	}
