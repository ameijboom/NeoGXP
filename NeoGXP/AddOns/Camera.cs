...
public class Camera : GameObject {
	...
	public bool ScreenPointInWindow(int screenX, int screenY) {
		return
			screenX >= renderTarget.windowX &&
			screenX <= renderTarget.windowX + renderTarget.width &&
			screenY >= renderTarget.windowY &&
			screenY <= renderTarget.windowY + renderTarget.height;
	}
	...
	public Vec2 ScreenPointToGlobal(int screenX, int screenY) {
		float camX = screenX - renderTarget.centerX;
		float camY = screenY - renderTarget.centerY;
