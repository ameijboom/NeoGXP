...
/// <summary>
/// A class for automatically creating NeoGXP sprites from Tiled files.
/// </summary>
public class TiledLoader {
	...
	/// </summary>
	public int NumTileLayers => map.Layers?.Length ?? 0;

	/// <summary>
	/// Returns the number of image layers in the loaded map.
	/// </summary>
	public int NumImageLayers => map.ImageLayers?.Length ?? 0;

	/// <summary>
	/// Returns the number of object groups in the loaded map.
	/// </summary>
	public int NumObjectGroups => map.ObjectGroups?.Length ?? 0;


	private string _foldername;
	private List<string> _manualObjects;
	...
	/// Call LoadTileLayers, LoadImageLayers and LoadObjectLayers to create NeoGXP sprites from the
	...
	//--------------------------------------------------------------------------------------------
	// REGION: public static utility methods, related to creating and placing NeoGXP objects
	...
	/// <param name="addCollider">Should a collider be added?</param>
	/// <param name="highQualityText">Should the text look nice and crisp?</param>
	/// <returns></returns>
	public static EasyDraw CreateTextField(TiledObject obj, bool addCollider = true, bool highQualityText = true) {
		...
		EasyDraw message = new(Mathf.Ceiling(obj.Width * scaleMultiplier), Mathf.Ceiling(obj.Height * scaleMultiplier), addCollider);
		// TODO: Cache fonts?

		// Set Font:
		SKFontStyle f = obj.textField.bold switch
		{
			1 when obj.textField.italic == 1 => SKFontStyle.BoldItalic,
			0 when obj.textField.italic == 1 => SKFontStyle.Italic,
			1 when obj.textField.italic == 0 => SKFontStyle.Bold,
		...
		// Set text alignment:
		message.TextAlign(
			obj.textField.horizontalAlign switch
			{
				"left" => CenterMode.Min,
				"right" => CenterMode.Max,
				_ => CenterMode.Center,
			},
			obj.textField.verticalAlign switch
			{
				"top" => CenterMode.Min,
				"bottom" => CenterMode.Max,
				_ => CenterMode.Center,
			}
		);
	...
	public static void DrawText(EasyDraw textCanvas, string text) {
		...
		// Set Text alignment:
		float textX =
			textCanvas.HorizontalTextAlign switch
			{
				CenterMode.Min => 0,
				CenterMode.Max => pixelWidth,
				_ => pixelWidth/2f,
			};
		float textY =
			textCanvas.VerticalTextAlign switch
			{
				CenterMode.Min => 0,
				CenterMode.Max => pixelHeight,
				_ => pixelHeight/2f,
			};
	...
	// NeoGXP objects from them. These methods use the state variables from TiledLoader
	...
	private void LoadObjectGroup(int index, bool autoInstance=false) {
			...
			if (_manualObjects.Contains(obj.Type)) {
				...
				if (autoInstance && obj.Type!=null) {
					try {
						...
						Console.WriteLine($"Couldn't automatically create an AnimationSprite object from the Tiled (image) object with ID {obj.ID} and type {obj.Type}.\n Error: {error.Message}");
				...
				if (autoInstance && obj.Type!="") {
					try {
						...
						Console.WriteLine($"Couldn't automatically create a sprite object from the Tiled (shape) object with ID {obj.ID} and type {obj.Type}.\n Error: {error.Message}");
						// Check the console for more information!
						throw;
					}
				}
	...
	private void LoadTileLayer(int index) {
